...
ApplicationWindow {
  id: window
  visible: true
  width: settings.loadSize("window/size").width
  height: settings.loadSize("window/size").height
  // color: "transparent"
  ...
  Component.onCompleted: {
    if (settings.loadBool("window/frame_less")) {
      window.flags |= Qt.FramelessWindowHint
    }
    if (settings.loadBool("window/full_screen")) {
          ...
          Item {
            ScrollView {
              ...
              Text {
                ...
                onVisibleChanged: {
                  if (visible) {
                    telemetrySocket.send_message("NetworkInterfaceInfo")
                  }
                }
              }
            }
    ...
    Row {
      ...
      Item {
        ...
        StackLayout {
          ...
          currentIndex: bar.currentIndex
          PipelineInfo {
            pipelineInfo: JSON.parse(videoPipelineManager.get_pipeline_info()).elements.reverse()
          }
          PipelineInfo {
            pipelineInfo: transmitterVideoPipelineInfo
            onVisibleChanged: {
              if (visible) {
                telemetrySocket.send_message("VideoPipelineInfo")
              }
            }
          }
        }
    ...
    Row {
      ...
      Item {
        ...
        StackLayout {
          ...
          currentIndex: bar.currentIndex
          PipelineInfo {
            pipelineInfo: audioPipelineManager ? JSON.parse(audioPipelineManager.get_pipeline_info()).elements.reverse() : null
          }
          PipelineInfo {
            pipelineInfo: transmitterAudioPipelineInfo
            onVisibleChanged: {
              if (visible) {
                telemetrySocket.send_message("AudioPipelineInfo")
              }
            }
          }
        }
        ...
        StackLayout {
          ...
          Item {
            Column{
              ...
              ConfigFieldString {
                key: "wifi/dhcp_range_start"
              }
              ConfigFieldString {
                key: "wifi/dhcp_range_end"
              }
              ConfigFieldString {
                key: "wifi/subnet_mask"
              }
              ConfigFieldInt {
                key: "wifi/dnsmasq_service_port"
              }
            }
          }
          Item {
